...
from detector3_1 import palline
...
def ObjectDetector():
    ...
    while 1:
        while StatoDetector:
            imgDetector,bianca,nera,ostacolo,bump=palline()
            #print('running')
            update=1
...
def DXred():
    ...
    while 1:
        ...
        if(bump != (0,0)):
            print(bump)
            yBumpMin = np.clip(bump[0],200,300)-200
            yBumpMax = np.clip(bump[1],200,300)-200
            bordi[yBumpMin:yBumpMax,bump[2]:bump[3]] = 255
...
def RaccoltaPallina(tipo):
    ...
    while 1:
        TinFPS = time.time()
        img,bianca,nera,ostacolo,bump=palline()
        if(tipo=='nera'):
            coordinate = nera
        else:
            coordinate = bianca
...
def stanza():
    ...
    while 1:
        ...
        if(bump!=(0,0)):
            img = cv2.rectangle(img, (bump[2],bump[0]), (bump[3],bump[1]), (0, 0, 255), 4)
