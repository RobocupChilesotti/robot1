...
def draw_rect(np_image, box, object_name, score):
    ...
    cv2.rectangle(np_image, (x_min, y_min), (x_max, y_max), (255, 0, 255), 2)
    ...
    cv2.rectangle(np_image, (x_min, label_ymin - labelSize[1] - 10), (x_min + labelSize[0], label_ymin + baseLine - 10),
                  (255, 255, 255), cv2.FILLED)  # Draw white box to put label text in
    cv2.putText(np_image, label, (x_min, label_ymin - 7), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0),
                2)  # Draw label text
...
def main():
    ...
    while cap.isOpened():
        ...
        interpreter.set_tensor(input_details[0]['index'], [cv2.resize(frame, (width,height))])
        ...
        for index, score in enumerate(scores[0]):
            if score > 0.6:
                ...
                draw_rect(frame, rects[0][index], object_name, score)
